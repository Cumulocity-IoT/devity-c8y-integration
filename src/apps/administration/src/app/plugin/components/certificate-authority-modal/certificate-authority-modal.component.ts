...
import { CertificateAuthorityFormlyResult, CertificatePolicyFormlyResult, Step2FormlyFieldConfig, Step3FormlyFieldConfig } from './formly-templates.model';
...
export class DevityCertificateAuthorityModalComponent {
  ...
  stepTitles = ['Basic Details', 'CA Properties', 'Certificate Policy', 'Device Selectors'];
  currentStep = 0;
  ...
  next(): void {
    ...
    if (this.currentStep === this.CA_PROPERTIES_STEP || this.currentStep === this.CERT_POLICY_STEP) {
  ...
  async submit(): Promise<void> {
    ...
    try {
      ...
      const config = this.formlyHelper.convertFormlyModelToPayload({ ...this.step2Model, ...this.step3Model });
      ...
      const createdCA = await this.proxy.createCertificateAuthority(caName, config);
      ...
      const c8yConfig =  {
        ...
        caId: caWithId.caId,
        cloudCaFingerprintPrimary: '5622207e1ba285f172756f6019af92ac808ed63286e24dfecc1e79873fb5d140f1ceb7133f2476e89a5f75f711f9813a9fbb8fd5287f64adfdcc53b864f9bdc5',
        cloudCaFingerprintSecondary: null,
        useOsTrustAnchor: true
      };
      console.log('Creating Cumulocity config...');
      const c8yConfigResponse = await this.proxy.createCumulocityConfig(c8yConfig);
      console.log('Creating Certificate template...')
      const certTemplateResponse = await this.proxy.createCertificateTemplate(config.defaultCertificateTemplate);
      ...
      const thinEdgeConfigResponse = await this.proxy.createThinEdgeConfig(thinEdgeConfig);
      ...
      await this.keynoaSerice.set({ ...this.pkiProvider, caId: caWithId.caId, c8yConfigId:  c8yConfigResponse.id, certTemplateId: certTemplateResponse.id, thinEdgeConfigId: thinEdgeConfigResponse.id });

      this.alert.success('Creation of Certificate Authority successful.');

    } catch (error) {
      this.alert.danger('Creation of Certificate Authority failed.', `${error}`);
      console.error(error);
    } finally {
