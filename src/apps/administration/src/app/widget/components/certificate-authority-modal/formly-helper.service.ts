...
export class FormlyHelperService {
  convertFormlyModelToPayload(
    model: CertificateAuthorityFormlyResult & CertificatePolicyFormlyResult
  ...
  private createDefaultCertificateTemplate(
    model: CertificatePolicyFormlyResult
  ): CertificateTemplate {
  ...
  private parseAlgorithm(algorithm: string): {
    keyType: string;
    keyBits: number;
  } {
