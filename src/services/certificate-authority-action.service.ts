...
export class CertificateAuhtorityActionService {
    ...
    async create(caName: string, authoritiy: CertificateAuthorityConfig): Promise<ActionResult> {
        try {
            try {
                await this.devityProxyService.createCertificateAuthority(caName, authoritiy);
                  this.alertService.success(
                    `Certificate Authority ${caName} created. It might take several minutes for the action to be completed.`
                  );
                  return { status: 'success' };
            } catch (e) {
                this.alertService.danger('Could not create Certificate Authority.', e as string);
    ...
    async delete(caCertificateId: number): Promise<ActionResult> {
        try {
            ...
            try {
                await this.devityProxyService.deleteCertificateAuthority(caCertificateId);
            } catch (e) {
                this.alertService.danger('Could not delete Certificate Authority.', e as string);
                return { status: 'error', error: e };
            }
            return { status: 'success' };
        } catch (e) {
